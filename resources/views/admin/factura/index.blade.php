       ...
       <div class="container-fluid">
            ...
            <div class="col-lg-12">
              ...
              <div class="card-header">
                Listado de facturas
                <a href="{{ url('facturas/create') }}" class="btn btn-primary" style="float: right !important;"><i class="mdi mdi-plus"></i> Registrar nueva factura</a>
              </div>
                            ...
                            <tr>
                              ...
                              <td>
                                @if ($element->titular_beneficiario <> null )
                                  {{ $element->titular->tx_nombres.' '. $element->titular->tx_apellidos }}
                                  @else
                                   {{ $element->beneficiarios->tx_nombres.' '. $element->beneficiarios->tx_apellidos }}
                                @endif
                              </td>
                               <td>
                                  @if ($element->titular_beneficiario <> null )
                                  {{ $element->titular->cedula }}
                                  @else
                                    {{ $element->beneficiarios->cedula }}
                                @endif
                              </td>
                               <td>
                                {{ number_format( $element->base_importe) }}
                              </td>
                               <td>
                                {{ number_format( $element->iva) }}
                              </td>
                               <td>
                                {{ number_format( $element->total_factura) }}
                              </td>
                              <td>
                              ...
                              @php
                                $autorizaciones = \DB::table('autorizaciones')->where('status',1)->where('factura_id',$element->id)
                                ->first();
                              @endphp
                              <td>
                                 ...
                                 @else
                                     <a href="{{ url('autorizacion',[$element->id,'factura']) }}" class="btn btn-sm btn-danger"></i> Solicitar autorización</a>
                                 @endif
                              </td>
                            </tr>
                          @endforeach
