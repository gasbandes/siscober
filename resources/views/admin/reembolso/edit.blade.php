...
<div class="page-content">
     @include('sweetalert::alert')
       ...
       <form role="form" id="facturas-form">
            <input type="hidden" id="_url" value="{{ url('reembolsos', [$reembolso->id]) }}">
            <input type="hidden" id="_token" value="{{ csrf_token() }}">
    				<div class="card">
    					<div class="card-header">
    						Modificar datos del reembolso
    						 <a href="{{ url('reembolsos') }}" class="btn btn-primary" style="float: right !important;"><i class="mdi mdi-plus"></i> Lista de facturas</a>
    					</div>
                ...
                <div class="form-group mt-3">
                  <label>Nro Factura</label> 
                   <input type="text" name="nu_factura" class="form-control input-sm" placeholder="Ingresa el número de factura " id="nu_factura" value="{{ $reembolso->nu_factura }}">
              ...
              <div class="col-lg-4">
                <div class="form-group mt-3">
                  <label>Nro Control</label>
                   <input type="text" name="nu_control" class="form-control input-sm" placeholder="Ingresa el número de factura " id="nu_control" value="{{ $reembolso->nu_control }}">
              ...
              <div class="col-lg-4">
                  ...
                  <label>Fecha de factura</label>
                    <div>
                      <input type="date" name="fecha_factura" class="form-control input-sm" placeholder="Ingresa el número de factura " id="fecha_factura" value="{{ $reembolso->fecha_factura }}">
                        ...
                        <label>Titular</label>
                         <select name="titular_id" class=" select1 form-control input-md" id="select_titular">
                           ...
                           @foreach ($titulares as $element)
                            <option value="{{ $element->id}}"  {{ ($reembolso->titular_id == $reembolso->titular_id) ? 'selected' : '' }}>{{ $element->cedula.' '.  '| '.$element->tx_nombres.' '. $element->tx_apellidos.' | '.$element->ente}}</option>
                           @endforeach
                         </select>
                         ...
                         <select name="proveedor_id" class=" form-control input-md" id="select_proveedores">
                          ...
                          @endphp
                           ...
                           @foreach ($proveedores as $element)
                            <option value="{{ $element->id }}" {{ ( $reembolso->proveedor_id == $reembolso->proveedor_id) ? 'selected' : '' }}> {{ $element->razon_social }} </option>
                           @endforeach
                         </select>
                         <span class="missing_alert text-danger" id="razon_social_alert"></span>
                      </div>
                    ...
                    <div class="form-group mt-3">
                      <label> Base Imponible </label>
                       <input type="text" name="base_importe" class="form-control input-sm" value="{{ $reembolso->base_importe }}" placeholder="Ingresa la Base Imponible de factura " id="base_importe">
                   ...
                   <div class="col-lg-4">
                    <div class="form-group mt-3">
                      <label> IVA </label>
                       <input type="text" name="iva" class="form-control input-sm" value="{{ $reembolso->iva }}" placeholder="Ingresa el iva de la factura " id="iva">
                  ...
                  <div class="col-lg-4">
                    <div class="form-group mt-3">
                      <label> Monto Total de Factura </label>
                       <input type="text" name="monto_pagado" class="form-control input-sm" value="{{ $reembolso->monto_pagado }}" placeholder="Ingresa el monto pagado de la factura " id="monto_pagado">
                  ...
                  <div class="col-lg-12">
                    <div class="form-group mt-3">
                      <label> Monto Total Pagado </label>
                       <input type="text" name="total_factura" class="form-control input-sm" placeholder="Ingresa el monto total de la factura " id="total_factura" value="{{ $reembolso->total_factura }}">
                  ...
                  <div class="col-lg-12 text-center">
                    ...
                    <div class="checkbox icheck">
                      <label>
                        <input type="radio" name="status" value="1" {{  $reembolso->status == 1 ? 'checked' : '' }} > PAGADO&nbsp;&nbsp;
                        <input type="radio" name="status" value="2" {{  $reembolso->status == 2 ? 'checked' : '' }}> PENDIENTE POR PAGO&nbsp;&nbsp;
                        <input type="radio" name="status" value="3" {{  $reembolso->status == 3 ? 'checked' : '' }}> ANULADA&nbsp;&nbsp;
                      </label>
                    </div>
                  </div>
                  </div>
